...
pub fn snapshot_project(
    ...
    let project_location = project.folder_location();
    let full_path: Cow<'_, Path>;
    if let Ok(result) = project_location.normalize() {
        full_path = Cow::Owned(result.into());
    } else {
        log::trace!("Failed to normalize path {}", project_location.display());
        full_path = Cow::Owned(project_location.into());
    }
