    ...
    if let Some(symlink) = symlinks.get(canonical) {
        if symlink.reifed_path == path {
            ...
            let properties = hashmap! {
                "Value".to_owned() => symlink.id.into(),
            };
...
fn build_and_update(
    ...
    if let Some(found_snapshot) = snapshot_from_vfs(symlinks, context, vfs, build_from_path)? {
        let mut snapshot = found_snapshot;
        if snapshot.snapshot_id.is_none() {
            snapshot.snapshot_id = Some(Ref::new());
        }

        if let Some(symlink) = symlinks.get_mut(canonical) {
            symlink.point_to_id = snapshot.snapshot_id.unwrap();
        ...
        log::error!("Failed to get get snapshot result from {}", path.display());
