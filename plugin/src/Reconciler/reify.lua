		...
		if virtualInstance.ClassName == "MeshPart" and virtualInstance.Properties.MeshId then
			...
			if virtualInstance.Properties.CollisionFidelity then
				...
				if not collisionFidelityOk then
					return error("Not ok collisionFidelityOk")
				end
			end

			-- Try hacky tag tagging
			local tagsOk, tagData = decodeValue(virtualInstance.Properties.Tags, instanceMap)
			if tagsOk then
				local tags = {}
				local tagList = string.split(tagData, "\0")
			...
			if renderFidelity then
				...
				if not renderFidelityOk then
					return error("Not ok renderFidelity")
				end
			else
	...
	return promiseGetInstance(virtualInstance, instanceMap)
		:andThen(function(instance)
			...
			return allPromise(promises)
				:andThen(function()
					-- Hack decoding time!
					-- Can't really decode this data safely
					-- https://devforum.roblox.com/t/some-info-on-sharedstrings-for-custom-collision-data-meshparts-unions-etc/294588/14

					-- if instance:IsA("MeshPart") then
					-- 	for _, collisionFidelity in pairs(Enum.CollisionFidelity:GetEnumItems()) do
					-- 		local tag = "_RojoFidelity_" .. collisionFidelity.Name
					-- 		if game:GetService("CollectionService"):HasTag(instance, tag) then
					-- 			print("Setting", instance, collisionFidelity)
